 ...
 */
public class CustomPbfBlobDecoder implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(com.graphhopper.reader.osm.pbf.PbfBlobDecoder.class);
     ...
     */
    public CustomPbfBlobDecoder(String blobType, byte[] rawBlob, PbfBlobDecoderListener listener) {
...
//		long changesetId = 0;
    ...
    private void processWays(List<Osmformat.Way> ways, PbfFieldDecoder fieldDecoder) {
        // Replica-specific variable tracking artificial way IDs used to replace negative IDs
        int currentArtificialWayId = Integer.MAX_VALUE;

        for (Osmformat.Way way : ways) {
            Map<String, String> tags = buildTags(way.getKeysList(), way.getValsList(), fieldDecoder);

            long wayId = way.getId() < 0 ? currentArtificialWayId-- : way.getId();

            ReaderWay osmWay = new ReaderWay(wayId);
            osmWay.setTags(tags);
