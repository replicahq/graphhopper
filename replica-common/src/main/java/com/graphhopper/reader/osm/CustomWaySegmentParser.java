...
public class CustomWaySegmentParser {
    ...
    private EdgeHandler edgeHandler = (from, to, pointList, way, nodeTags, segmentIndex) ->
            System.out.println("edge " + from + "->" + to + " (" + pointList.size() + " points); segment index " + segmentIndex);
    private int workerThreads = 2;

    private final CustomOSMNodeData nodeData;
    private Date timestamp;

    private CustomWaySegmentParser(CustomOSMNodeData nodeData) {
    ...
    /**
     * Replica-added helper to get access to internal mapping of GH node ID to OSM node ID
     */
    public Map<Integer, Long> getGhNodeIdToOsmNodeIdMap() {
        if (nodeData.getNodeCount() == 0) {
            throw new IllegalStateException("Can't grab node data before way segment parser has been run!");
        }
    ...
    private class Pass2Handler implements ReaderElementHandler {
            ...
            for (int i = 0; i < parentSegment.size(); i++) {
                ...
                if (nodeData.isSplitNode(node.osmNodeId)) {
                    ...
                    if (!segment.isEmpty()) {
                        ...
                        handleSegment(segment, way, segmentIndex++);
                        segment = new ArrayList<>();
                    }
        ...
        void handleSegment(List<SegmentNode> segment, ReaderWay way, int segmentIndex) {
    ...
    public static class Builder {
        private final CustomWaySegmentParser waySegmentParser;
        ...
        public Builder(PointAccess pointAccess, Directory directory) {
            waySegmentParser = new CustomWaySegmentParser(new CustomOSMNodeData(pointAccess, directory));
        }
        ...
        public CustomWaySegmentParser build() {
    ...
    public interface EdgeHandler {
        void handleEdge(int from, int to, PointList pointList, ReaderWay way, List<Map<String, Object>> nodeTags, int segmentIndex);
    }
