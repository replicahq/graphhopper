 ...
 * @author Nop
 */
public class CustomOSMInputFile implements Sink, OSMInput {
    ...
    private CustomPbfReader pbfReader;
    private Thread pbfReaderThread;
    private boolean hasIncomingData;
    private int workerThreads = -1;
    private OSMFileHeader fileheader;

    private int currentArtificialWayId = Integer.MAX_VALUE;

    public CustomOSMInputFile(File file) throws IOException {
        bis = decode(file);
        itemQueue = new LinkedBlockingQueue<>(50_000);
        itemBatch = new ArrayDeque<>(MAX_BATCH_SIZE);
    }

    public CustomOSMInputFile open() throws XMLStreamException {
        if (binary) {
            openPBFReader(bis);
        } else {
            openXMLStream(bis);
     ...
     */
    public CustomOSMInputFile setWorkerThreads(int threads) {
                ...
                if (idStr != null) {
                    ...
                    switch (name.charAt(0)) {
                        ...
                        case 'w': {
                            ...
                            // Custom replica-specific check to override negative way values with new
                            // artificial IDs starting at Integer.MAX_VALUE
                            if (id < 0) {
                                id = currentArtificialWayId--;
                            }
    ...
    private void openPBFReader(InputStream stream) {
        ...
        pbfReader = new CustomPbfReader(stream, this, workerThreads);
