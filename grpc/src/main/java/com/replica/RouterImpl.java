...
public class RouterImpl extends router.RouterGrpc.RouterImplBase {
    ...
    public RouterImpl(GraphHopper graphHopper, PtRouter ptRouter, MatrixAPI matrixAPI,
        ...
        this.isochroneTriangulator = new JTSTriangulator(graphHopper.getRouterConfig());
    }
    ...
    public void routeIsochrone(IsochroneRouteRequest request, StreamObserver<IsochroneRouteReply> responseObserver) {
        ...
        IsochroneRouteReply.Builder replyBuilder = IsochroneRouteReply.newBuilder();
        for (int i = 0; i < isochrones.size(); i++) {
            Geometry isochrone = isochrones.get(i);
            replyBuilder.addBuckets(IsochroneBucket.newBuilder()
                    .setBucket(i)
